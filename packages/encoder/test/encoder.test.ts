...
describe("Wrapping, encoding, and overload resolution", () => {
  ...
  describe("Encoding", () => {
    ...
    describe("Transaction options", () => {
      ...
      it("In loose mode, encodes transaction options that could be type/value input", async () => {
        const result = await encoder.encodeTransaction(
          abi,
          [
            {
  ...
  describe("Overload resolution", () => {
    ...
    describe("Overall priority", () => {
      ...
      let abisUint8ArrayInput: Abi.FunctionEntry[];
      beforeAll(() => {
        ...
        abisUint8ArrayInput = <Abi.FunctionEntry[]>(
          Abi.normalize(artifacts.TestContract.abi).filter(
            entry => entry.type === "function" && entry.name === "overloadedUint8ArrayInput"
          )
        );
      });
    ...
    describe("Overload resolution and loose matching", () => {
      let abis: Abi.FunctionEntry[];
      let abisUnambiguous: Abi.FunctionEntry[];
      beforeAll(() => {
        abis = <Abi.FunctionEntry[]>(
          Abi.normalize(artifacts.TestContract.abi).filter(
            entry =>
              entry.type === "function" && entry.name === "overloadedAmbiguous"
          )
        );
        abisUnambiguous = <Abi.FunctionEntry[]>(
          Abi.normalize(artifacts.TestContract.abi).filter(
            entry =>
              entry.type === "function" && entry.name === "overloadedUnambiguous"
          )
        );
      });
